      ...
      <div class="wrapper">
        ...
        <div class="inputs">
          <template v-for="(input, index) of inputs">
            ...
            <input :class="input.class" :id="'popup-' + input.type + '-' + index" :type="input.type" :value="input.value" :min="input.min" :max="input.max" :step="input.step" :accept="input.accept" :style="input.style" :title="input.title || input.value" :disabled="input.disabled" v-on:[input.event]="runCallback(input.callback, $event)">
          </template>
        </div>
      </div>
    </div>
